...
describe("figure-list component", () => {
  ...
  let el: DebugElement;
  let dataService: FigureDataService | jest.Mocked<FigureDataService>;

  const dataServiceMock = {
    getFigureItems: jest.fn().mockReturnValue({ subscribe: () => null }),
    removeFigureItem: jest.fn(),
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [{ provide: FigureDataService, useValue: dataServiceMock }],
    })
      .compileComponents()
      .then(() => {
  ...
  it("should receive event from list-item event emitter", () => {
    const onRemoveHandlerSpy = jest.spyOn(component, "onRemoveHandler");
